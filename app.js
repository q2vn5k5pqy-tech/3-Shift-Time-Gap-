...
(() => {
  ...
  const STRINGS = {
    ja: {
      ...
      offsetTitle: "端末時刻に足す時間（h）",
      offsetHint: "例：+1.5 / -6.5 / -14.5（小数は0.5刻み推奨）",
      reset: "初期値に戻す",
      save: "保存",
      close: "閉じる",
      orderLabel: "切替順（固定）",
    ...
    en: {
      ...
      offsetTitle: "Hours to add to device time",
      offsetHint: "Example: +1.5 / -6.5 / -14.5 (0.5 steps recommended)",
  ...
  // Defaults (minutes)
  const DEFAULT_OFFSETS = { early: 90, late: -390, night: -870 };
  ...
  function computeConvertedFromOffsets(offsetsMin) {
    const offsetMin = offsetsMin[state.selectedShift] ?? 0;
    return new Date(Date.now() + offsetMin * 60_000);
  }

  function renderPreviewFromInputs() {
    if (!el.previewText) return;
    ...
    const offsets = (early === null || night === null || late === null)
      ? state.offsetsMin
      : { early, night, late };

    const s = getStr();
    const deviceLabel = s.device || (state.lang === "ja" ? "端末" : "Device");
    const convertedLabel = s.converted || (state.lang === "ja" ? "換算" : "Converted");
    const device = new Date();
    const converted = computeConvertedFromOffsets(offsets);

    el.previewText.textContent = `${deviceLabel} ${formatHHMM(device)} → ${convertedLabel} ${formatHHMM(converted)}`;
  }
  ...
  function applyLanguage() {
    ...
    if (el.previewLabel) el.previewLabel.textContent = s.previewLabel;
    if (el.previewHint) el.previewHint.textContent = s.previewHint;
  ...
  function renderMeridiem(date) {
    if (!el.meridiem) return;
    const s = getStr();
    const key = getMeridiem(date);
    el.meridiem.textContent = key === "am" ? s.am : s.pm;
  }
  ...
  function renderClock(date) {
    ...
    // SVG transform attribute fallback (better Safari compatibility)
    if (el.minuteHand && el.minuteHand.setAttribute) el.minuteHand.setAttribute("transform", `rotate(${minuteAngle} 100 100)`);
    if (el.hourHand && el.hourHand.setAttribute) el.hourHand.setAttribute("transform", `rotate(${hourAngle} 100 100)`);
  }

  function renderAll() {
    const d = getConvertedDate();
    renderShiftUI();
    renderMeridiem(d);
    renderClock(d);
  }
  ...
  function bindEvents() {
    ...
    // Live preview updates while editing offsets
    [el.offsetEarly, el.offsetNight, el.offsetLate].forEach((inp) => {
